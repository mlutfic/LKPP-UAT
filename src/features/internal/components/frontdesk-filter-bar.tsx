...
export type FrontdeskFilterSection = "active" | "history";

export function FrontdeskFilterBar({
  section,
  onSectionChange,
  activeCount,
  historyCount,
  ...
  onSectionChange: (value: FrontdeskFilterSection) => void;
  activeCount: number;
  historyCount: number;
    ...
    <div className="space-y-4 rounded-[calc(var(--radius-2xl)+4px)] border border-border bg-surface-container-lowest p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.24)] sm:p-5">
            ...
            {[
              ...
              return (
                <button
                  key={tab.value}
                  type="button"
                  onClick={() => onSectionChange(tab.value as FrontdeskFilterSection)}
                  className={cn(
      ...
      <div className="relative">
        ...
        <AppInput
          ...
          placeholder="Cari nomor antrean, nama tamu, NIK, layanan, atau unit"
      ...
      {showAdvancedFilters ? (
        <div className="grid gap-3 border-t border-border/80 pt-4 md:grid-cols-2 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_220px]">
          <AppSearchSelect
            value={unitFilter}
